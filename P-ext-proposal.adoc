...
comes from the odd position of `rs2`.
...
comes from the even position of `rs2`.
...
WZIP8P writes its 64-bit result to the even-odd register pair specified by
`rd_p` (no write occurs if `rd_p = 0`). It is encoded in the RV32 register-pair
OP-IMM-32 format with `f=111` and `w=10`.
...
`rd_p` (no write occurs if `rd_p = 0`).  It is encoded in the RV32 register-pair
OP-IMM-32 format with `f=111` and `w=11`.
